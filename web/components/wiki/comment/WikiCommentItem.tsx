...
interface WikiCommentItemProps {
  comment: WikiComment;
  onUpdate?: () => void;
...
export function WikiCommentItem({
  comment,
  onUpdate,
  onError,
  currentUserId,
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await WikiService.updateComment(comment.id, {
  ...
  const handleDelete = async () => {
    if (confirmDelete) {
      setIsSubmitting(true);
      try {
        const res = await WikiService.deleteComment(comment.id);
        if (res.data) {
          onUpdate?.();
        } else {
