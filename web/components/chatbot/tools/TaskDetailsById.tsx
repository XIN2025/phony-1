...
const PureTaskDetailsByIdTool = ({ tool }: ToolProps) => {
  if (!tool.args) {
    return null;
  }
  ...
  return (
    <div className="my-1 flex w-fit items-center gap-1 rounded-md text-sm">
      ...
      <ShinyText text={`Getting task details...`} />
    </div>
  );
};

const TaskDetailsByIdTool = memo(PureTaskDetailsByIdTool, (prevProps, nextProps) => {
...
export default TaskDetailsByIdTool;
