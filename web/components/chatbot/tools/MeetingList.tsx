...
const PureMeetingListTool = ({ tool }: ToolProps) => {
  if (!tool.args) {
    return null;
  }
  ...
  return (
    <div className="my-1 flex w-fit items-center gap-1 rounded-md text-sm">
      ...
      <ShinyText text={`Getting list of meetings...`} />
    </div>
  );
};

const MeetingListTool = memo(PureMeetingListTool, (prevProps, nextProps) => {
...
export default MeetingListTool;
