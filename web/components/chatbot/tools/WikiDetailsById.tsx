...
const PureWikiDetailsByIdTool = ({ tool }: ToolProps) => {
  if (!tool.args) {
    return null;
  }
  ...
  return (
    <div className="my-1 flex w-fit items-center gap-1 rounded-md text-sm">
      ...
      <ShinyText text={`Getting wiki details...`} />
    </div>
  );
};

const WikiDetailsByIdTool = memo(PureWikiDetailsByIdTool, (prevProps, nextProps) => {
...
export default WikiDetailsByIdTool;
