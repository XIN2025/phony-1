...
const PureMeetingDetailsByIdTool = ({ tool }: ToolProps) => {
  if (!tool.args) {
    return null;
  }
  ...
  return (
    <div className="my-1 flex w-fit items-center gap-1 rounded-md text-sm">
      ...
      <ShinyText text={`Getting meeting details...`} />
    </div>
  );
};

const MeetingDetailsByIdTool = memo(PureMeetingDetailsByIdTool, (prevProps, nextProps) => {
...
export default MeetingDetailsByIdTool;
