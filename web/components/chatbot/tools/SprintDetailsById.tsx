...
const PureSprintDetailsByIdTool = ({ tool }: ToolProps) => {
  if (!tool.args) {
    return null;
  }
  ...
  return (
    <div className="my-1 flex w-fit items-center gap-1 rounded-md text-sm">
      ...
      <ShinyText text={`Getting sprint details...`} />
    </div>
  );
};

const SprintDetailsByIdTool = memo(PureSprintDetailsByIdTool, (prevProps, nextProps) => {
...
export default SprintDetailsByIdTool;
