...
import { MentionsRenderer } from '@/components/comments/MentionsRenderer';
import { BugsService } from '@/services/bugs.api';
import { BugComment } from '@/types/bug';

interface BugsCommentItemProps {
  comment: BugComment;
  onUpdate?: () => void;
...
export function BugsCommentItem({
  comment,
  onUpdate,
  onError,
  currentUserId,
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await BugsService.updateComment(comment.id, {
        content: editContent.trim(),
      });
      if (res.data) {
        onUpdate?.();
      } else {
        onError?.(res?.error?.message || 'Failed to update comment. Please try again.');
      }
      setIsEditing(false);
    } catch {
  ...
  const handleDelete = async () => {
    if (confirmDelete) {
      setIsSubmitting(true);
      try {
        const res = await BugsService.deleteComment(comment.id);
        if (res.data) {
          onUpdate?.();
        } else {
